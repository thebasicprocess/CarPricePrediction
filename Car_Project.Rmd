...
data_model <- data %>% select(-all_of(excluded_columns))

# Convert categorical variables to factors
categorical_vars <- names(data_model)[sapply(data_model, is.character)]
...
data_model <- data.frame(predict(dummies, newdata = data_model))
...
trainIndex <- createDataPartition(data_model$price, p = 0.8, list = FALSE)
...
cat("The linear regression model has been built and evaluated.\n")
cat(sprintf("RMSE on Test Data: %.2f\n", rmse))
cat(sprintf("R-squared on Test Data: %.2f\n", r_squared))
